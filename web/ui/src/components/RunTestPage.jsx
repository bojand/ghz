...
export default function RunTestPage () {
  ...
  const [autoGenerated, setAutoGenerated] = useState(false) // mark if data is auto-generated, can be reset after user modification
  ...
  // success result
  const [resultReportId, setResultReportId] = useState(null)
        ...
        setJobStatus(j.status)
          if (j.status === 'SUCCEEDED') {
            ...
            if (j.result && j.result.report && j.result.report.id) {
              setResultReportId(j.result.report.id)
            }
    ...
    try {
      ...
      if (res.status === 202) {
        ...
        if (data && data.report && data.report.id) {
          setResultReportId(data.report.id)
        } else {
  ...
  return (
    <Pane padding={16}>
      ...
      <Pane marginBottom={12}>
        ...
        {projectId && <Text marginTop={4} display='block' size={300} color='muted'>could not modify</Text>}
      </Pane>
      ...
      {resultReportId && !error && (
        <Alert intent='success' title='Run Finished' marginBottom={12}>
          <Pane display='flex' alignItems='center' gap={12}>
            <Text size={300}>Report ID: {resultReportId}</Text>
            <Button height={24} onClick={() => history.push(`/reports/${resultReportId}`)}>View Report</Button>
          </Pane>
        </Alert>
      )}
